...
bool load_step(const char*, Model*) {
    throw RuntimeError("STEP import is not available in the host build.");
}
} // namespace Slic3r
#else
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 7. Root CMakeLists.txt — append orca-wasm bridge + WASM link target injection.
...
if orca_root_cmake.exists():
    ...
    if "orca-wasm WASM bridge" not in content:
        if not DRY_RUN:

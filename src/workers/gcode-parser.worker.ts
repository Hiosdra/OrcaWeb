...
function parseGcode(gcode: string): ParseResult {
  ...
  const markerLayers: LayerAcc[] = [],
    layerMap = new Map<number, LayerAcc>()
  ...
  while (position < gcode.length) {
    ...
    if (isLinear) {
      ...
      for (let k = 0; k < points.length; k += 3) {
        addSegment(!hasE, px, py, pz, points[k], points[k + 1], points[k + 2])
        px = points[k]
        py = points[k + 1]
        pz = points[k + 2]
      }
  ...
  const centerX = Number.isFinite(minX) ? (minX + maxX) / 2 : 0,
    centerY = Number.isFinite(minY) ? (minY + maxY) / 2 : 0
  const maxR = Number.isFinite(minX) ? Math.hypot((maxX - minX) / 2, (maxY - minY) / 2) : 10
  const accs = hasLayerMarkers
    ? markerLayers.filter((layer) => layer.features.size > 0 || layer.travels.length > 0)
  ...
  const layers = accs.map((data): Layer => {
    const features = Array.from(data.features.entries()).map(([type, points]) => {
      ...
      for (let i = 0; i < points.length; i += 3) {
        segments[i] = points[i] - centerX
        segments[i + 1] = points[i + 1] - centerY
        segments[i + 2] = points[i + 2]
      }
      return { type, segments }
    })
    const travels = new Float32Array(data.travels.length)
    for (let i = 0; i < travels.length; i += 3) {
      travels[i] = data.travels[i] - centerX
      travels[i + 1] = data.travels[i + 1] - centerY
      travels[i + 2] = data.travels[i + 2]
    }

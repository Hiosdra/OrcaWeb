...
//
// Deliberately unparallelized even in shims-mt/ (this is the multithreaded
// shim directory — see orca-wasm/MT-PLAN.md "Explicitly out of scope").
// Correctly parallelizing a staged pipeline needs bounded producer/consumer
// queues between stages and gets deadlock-prone fast; libslic3r only uses
// this for G-code export (8 call sites, not the main slicing bottleneck —
// see MT-PLAN.md Phase 0), so the risk/reward doesn't justify it for v1.

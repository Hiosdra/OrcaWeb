...
The multithreaded engine keeps orca-wasm's C bridge API unchanged. OrcaSlicer
...
artifact mirror (the primary deployment) ever serves — a single Emscripten WASM
binary cannot switch pthread support at runtime, and artifact mirror cannot send
...
## Deployment: cross-origin host (COOP/COEP) + artifact mirror (ST-only)
...
`Cross-Origin-Embedder-Policy: require-corp` response headers. artifact mirror
cannot send custom headers at all, so it always serves the ST engine
(`slicer.js`/`slicer.wasm`) and never MT.

The cross-origin host mirror sends both headers via `public/_headers`, so it's the
only host where `self.crossOriginIsolated` is ever true. `deploy.yml` mirrors
`slicer-mt.js`/`slicer-mt.wasm` onto artifact mirror alongside the ST engine
(best-effort — a missing MT release never fails the deploy); cross-origin host
...
cross-origin host build points `WASM_BASE_URL` at the artifact mirror copies and
loads them cross-origin. That only works because artifact mirror serves them
...
cannot be read cross-origin directly and are never fetched by the host at
runtime.
...
making the MT engine 100% non-functional in the host despite compiling
...
diffed) — roughly a quarter of one core. The host was near-idle while the
host integration said "Slicing…", i.e. threads asleep in `Atomics.wait`, not computing — a
...
cross-origin host COOP/COEP deployment only after that published engine is live — and
...
Measured in Node (same engine binaries the host loads) on a 16-logical-core
...
- The pool/cap fix is verified by the pool-bump reproduction above, but the
  ...
  that before enabling the cross-origin host path.
- oneTBB's scheduler on Emscripten pthreads has not been proven under
  irregular/highly unbalanced workloads in-host — only the fixed
...
- Large real-world models remain the most important source of performance
  ...
  or memory use.
- The cross-origin host→GitHub-Pages cross-origin load path is exercised by CI's
  ...
  real cross-origin host deploy, not locally. Note the pthread pool is spawned from
  ...
  cross-origin host regardless of headers. The Blob form is the standard cross-origin
